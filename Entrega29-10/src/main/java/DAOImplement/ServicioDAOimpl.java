...
public class ServicioDAOimpl extends BaseDAOimpl<Servicio> implements ServicioDAO{
	...
	public List<Servicio> buscarServicioPorCategoria(String categoria) {
		Query consulta = EMFSingleton.getEntityManagerFactory().createEntityManager().createQuery("select e from Servicio e INNER JOIN TipoServicio ts ON e.tipoServicio=ts.id WHERE ts.nombre = :categoria");
